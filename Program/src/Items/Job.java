 ...
 */
public class Job extends AddData{
     ...
     public Job(int jobNumber)
    {
        ...
        if(jobNumber == 1) {arrivTime =0;}    // first job always arrive at time = 0
        else {arrivTime = rand.nextInt(30)+1;}  
    ...
    public int getTurnaround(int SimulationTime){
        if(finished){  // if job is finished
            return (finish - arrivTime );
        }
        if(SimulationTime > arrivTime){ // if job arrived but hasn't finished yet
           return (SimulationTime - arrivTime);
        }
    ...
    public boolean isFirst(Job other) {
        if(this.arrivTime == other.arrivTime) // if both have the same arrive time
        {
            return (this.jobNumber < other.jobNumber);
        }
        return (this.arrivTime < other.arrivTime);
    }
    ...
    public void ShowData()
    {
        ...
        System.out.println("# = " + this.jobNumber + " , arrive = " + this.arrivTime + " , burst = " + this.burst);
    }
}
