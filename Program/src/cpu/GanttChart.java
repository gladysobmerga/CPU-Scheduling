...
public class GanttChart {
    // gantt chart
    private static int ganttX =20 ; // start drawing point on x-coordinate
    private static final int ganttY = 411; // Gantt location on y-coordinate 
    ...
    public static void addJob(Job job , int time){
       ...
       ganttX += 6; // set next job location
       List.add(cell); // add cell to gantt chart list
       if( (time+1) % 10 == 0 ) 
       {
           List.add(Cell.createMark(ganttX -1 , ganttY+50));  // put small black mark every 10 times 
       }
    }
    ...
    public static void clear(){
         List.clear();
         ganttX = 20;  // start location on x-coordinate
