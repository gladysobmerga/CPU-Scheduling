...
public class Simulation extends Face {
    ...
    {
		if(AlgorithmType == FCFS){myAlgorithm = new FCFS(MainQueue.get());}  // first come first served
        else if(AlgorithmType == SJF) {myAlgorithm = new SJF(MainQueue.get());} // shortest job first
        else if(AlgorithmType == Priority1) {myAlgorithm = new Priority1(MainQueue.get());} // priority non-preemptive
        else if(AlgorithmType == STRF) {myAlgorithm = new STRF(MainQueue.get());} // shortest time remaining first
        else if(AlgorithmType == Priority2) {myAlgorithm = new Priority2(MainQueue.get());} // priority preemptive
        else if(AlgorithmType == RR) {myAlgorithm = new RR(MainQueue.get() , Quantum);}  // Round Robin
    }
}
